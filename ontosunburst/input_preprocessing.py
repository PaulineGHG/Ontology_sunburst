...
import os
import networkx
from typing import Tuple, List, Dict, Set, TypeAlias, Literal, cast, get_args
...
#                                             FUNCTIONS
# ==================================================================================================
def check_inputs_sets(interest: Input, reference: Input | None) -> Tuple[InputsAb, InputsAb]:
...
def manage_unmapped(interest: InputsAb, reference: InputsAb, unmapped: Set[str], manage='r')\
        -> Tuple[InputsAb, InputsAb, Set[str]]:
    """ Manage unmapped value (remove values by default) and returns interest, reference and
    all_concepts according to it.
    (For now, only remove values with 'r' manage parameter but will maybe integrate other
    management alternatives in the future)
    ...
    for u in unmapped:
        if manage == 'r':
            ...
            if u in reference:
                reference.pop(u)
    return interest, reference, set(interest.keys()).union(set(reference.keys()))
...
def get_id_to_label_dict(ontology: OntologyName | None,
    ...
    """ Return the id to labels dict associating an ontology ID to a more explicit label depending
    on the inputs given (ontology name, custom ontology dict or custom ontology json file path)

    Parameters
    ----------
    ontology: str | None
        A default ontology name (chebi, metacyc, ec, ...) or None for custom ontology
    id_to_label_input: dict[str, str] | str | None
        A custom id to labels as a dict or a json file path or None for default ontology or no
        labels

    Returns
    -------
    dict[str, str]
        Id to labels dictionary associating an ontology ID to a more explicit label.
    """
    id_to_label = dict()
