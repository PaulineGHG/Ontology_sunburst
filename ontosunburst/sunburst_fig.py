...
def get_fig_parameters(classes_abondance: Dict[str, int], parent_dict: Dict[str, List[str]],
                       children_dict: Dict[str, List[str]], root_item, full: bool = True, names: Dict = None) \
        -> Dict[str, List]:
    ...
    data = {'ID': list(),
            ...
            'Label': list(),
            'Count': list()}
    for m, v in classes_abondance.items():
        if m != root_item:
            ps = parent_dict[m]
            if names is None:
                data = add_value_data(data, m, m, v, ps[0])
            else:
                data = add_value_data(data, m, names[m], v, ps[0])
            if len(ps) > 1 and full:
                for p in ps[1:]:
                    o = '__' + p
                    m_id = m + o
                    data = add_value_data(data, m_id, m, v, p)
                    if m in children_dict.keys():
                        cs = children_dict[m]
                        for c in cs:
                            data = add_children(data, o, c, m_id, classes_abondance, children_dict)
        else:
            data = add_value_data(data, m, m, v, '')
...
def get_data_enrichment_analysis(data, b_classes_abundance, test):
    M = np.max(list(b_classes_abundance.values()))
    m_list = [b_classes_abundance[x] if x in b_classes_abundance.keys() else 0 for x in data['Label']]
    N = np.max(data['Count'])
    n_list = data['Count']
    data['p_value'] = list()
    nb_classes = len(set(data['Label']))
    ...
    for i in range(len(m_list)):
        ...
        # Hypergeometric Test
            ...
            raise ValueError(f'test parameter must be in : [Binomial, Hypergeometric]')
...
def generate_sunburst_fig(data: Dict[str, List[str or int or float]], output: str, sb_type: str,
    ...
    if sb_type == 'proportion':
        fig = px.sunburst(data, names='Label', parents='Parent', values='Count', ids='ID', color='Proportion',
                          template='presentation', color_continuous_scale=px.colors.sequential.Mint, maxdepth=7)
    elif sb_type == 'comparison':
        data, signif = get_data_enrichment_analysis(data, b_classes_abond, test)
        m = np.mean(data['Proportion Difference'])

        fig = make_subplots(rows=1, cols=2,
                            column_widths=[0.3, 0.7],
                            vertical_spacing=0.03,
                            subplot_titles=('Significant p-values', 'Metabolites classes enrichment representation'),
                            specs=[[{'type': 'table'}, {'type': 'sunburst'}]])

        fig.add_trace(go.Sunburst(labels=data['Label'], parents=data['Parent'], values=data['Count'],
                                  ids=data['ID'], hovertext=[f'P value: {10**(-x)}' if x > 0 else f'P value: {10**x}'
                                                             for x in data['p_value']],
                                  hoverinfo='label+value+percent entry+text', maxdepth=7,
                                  marker=dict(colors=data['p_value'],
                                              cmid=m, showscale=True)), row=1, col=2)
